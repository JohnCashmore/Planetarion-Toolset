...
use App\Alliance;
use App\Politics;
use App\AttackTarget as AttackTargetClass;
use App\Planet as PlanetClass;
use App\AttackBooking as AttackBookingClass;
...
class AttackController extends ApiController
{
	...
	public function show($id, Request $request)
	{
		...
		foreach($attack['targets'] as $key => $target) {
			...
			$politicalRelation = Politics::where('allianceId', $target['planet']['alliance_id'])->first();
			if (is_object($politicalRelation)):
				
				// calculate number of planets
				$attackBookingClass = AttackBookingClass::where('attack_id', $attack['id'])->whereNotNull('user_id')->get();
				foreach ($attackBookingClass as $keyInfo => $attackBooking):
					$planetId = AttackTargetClass::where('attack_id', $attack['id'])->where('id', $attackBooking->attack_target_id)->first()->planet_id;
				...
				$attackTargetClass = AttackTargetClass::where('attack_id', $attack['id'])->where('planet_id',  $target['planet']['id'])->first();	
				$numberOfWaves = AttackBookingClass::where('attack_id', $attack['id'])->where('attack_target_id',  $attackTargetClass->id)->whereNotNull('user_id')->count();
				
				/*echo '--Planet ' . $target['planet']['id'] . ' (' . $target['planet']['coords'] . ')----' . PHP_EOL;
				echo 'alliance id (from $target): ' . $target['planet']['alliance_id'] . PHP_EOL;
				echo 'number of planets taken: ' . $numberOfPlanets  . PHP_EOL;
				echo 'number of waves taken: ' . $numberOfWaves  . PHP_EOL;
				echo 'max planets in politics: ' . $politicalRelation->maxPlanets . PHP_EOL ;
				echo 'max waves in politics: ' . $politicalRelation->maxWaves . PHP_EOL;
				*/
				//echo "<pre>";var_dump($registeredPlanets, $target['planet']['id']);
				if (
					($politicalRelation->status == 'nap') || 
					($numberOfPlanets >= $politicalRelation->maxPlanets && !isset($registeredPlanets[$target['planet']['id']])) || 
					($numberOfWaves >= $politicalRelation->maxWaves)
				):
	...
	public function book($attackId, $bookingId, Request $request, MyBookings $bookings)
	{
		if($attack = Attack::where(['attack_id' => $attackId, 'is_opened' => 1, 'is_closed' => 0])->first()) {
			...
			$attackBooking = AttackBookingClass::where('id', $bookingId)->first();
			
			$attackTarget = AttackTargetClass::where('attack_id', $attack['id'])->where('id', $attackBooking->attack_target_id)->first();
			
			$planetId = $attackTarget->planet_id;
			$planet = Planet::where('id', $planetId)->first();
			$politicalRelation = Politics::where('allianceId', $planet->alliance_id)->first();
			
			if (is_object($politicalRelation)):
				
				// calculate number of planets
				$attackBookingContainer = AttackBookingClass::where('attack_id', $attack['id'])->whereNotNull('user_id')->get();
				foreach ($attackBookingContainer as $attackBookingValue):
					$planetId = AttackTargetClass::where('attack_id', $attack['id'])->where('id', $attackBookingValue->attack_target_id)->first()->planet_id;
				...
				// calculate number of waves
				$attackTargetClass = AttackTargetClass::where('attack_id', $attack['id'])->where('planet_id',  $planet->id)->first();	
				$numberOfWaves = AttackBookingClass::where('attack_id', $attack['id'])->where('attack_target_id',  $attackTargetClass->id)->whereNotNull('user_id')->count();

				if (
					($politicalRelation->status == 'nap') || 
					($numberOfPlanets >= $politicalRelation->maxPlanets && !isset($registeredPlanets[$planet->id])) || 
					($numberOfWaves >= $politicalRelation->maxWaves)
				):
					// do nothing
				else:
