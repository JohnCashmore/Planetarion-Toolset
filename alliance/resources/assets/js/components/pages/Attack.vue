							...
							<tbody>
								<tr v-for="(target, number) in attack.targets">
									...
									<td class="text-center align-middle book" v-for="booking in target.bookings">
										<span v-if="!target.closed && !booking.user_id && booking.user_id != user.id">
											<a v-on:click="book(number, target.id, booking.id)" class="btn-link btn-sm"><i class="fas fa-book"></i></a>
										</span>
										<span v-if="target.closed && !booking.user_id && booking.user_id != user.id">
											<button class="btn btn-sm btn-danger"><i class="fa fa-error fa-minus-circle"></i></button>
										</span>
				...
				.then((response) => {
					if (response.error) {
						this.$notify({
							group: 'foo',
							title: 'Error',
							text: 'Maximum bookings for alliance reached',
							type: 'success'
						});
					} else {
